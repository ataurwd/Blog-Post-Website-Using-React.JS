...
const SingleBlog = ({ data, handleBookmark,handelMarkAsRead }) => {
    ...
    return (
        <div className="my-10 space-y-3">
            ...
            <div className="flex justify-between">
                ...
                <div className="flex items-center">
                    <p>{reading_time}</p>
                    <button onClick={() => handleBookmark(data)} className="ml-2">
            ...
            <p>
                {hashtags.map((hash, index) => (
                    <a key={index} href="">{hash}</a>
                ))}
            </p>
            <button onClick={() => handelMarkAsRead(reading_time)} className="text-[#6047EC]">Mark as read</button>
        </div>
    );
};
